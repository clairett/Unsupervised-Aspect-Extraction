...
def create_model(args, maxlen, vocab):
    ...
    r_s = WeightedAspectEmb(args.aspect_size, args.emb_dim, name='aspect_emb', embeddings_regularizer=ortho_reg)(p_t)
    ...
    if args.emb_path:
        ...
        emb_reader = EmbReader(args.emb_path, vocab, emb_dim=args.emb_dim)
        logger.info('Initializing word embedding matrix')
        word_emb = model.get_layer('word_emb').embeddings
        K.set_value(word_emb, emb_reader.get_emb_matrix_given_vocab(vocab, K.get_value(word_emb)))
        logger.info('Initializing aspect embedding matrix as centroid of kmean clusters')
        aspect_emb = model.get_layer('aspect_emb').embeddings
        K.set_value(aspect_emb, emb_reader.get_aspect_matrix(args.aspect_size))
